...
**Rollover（续期）**:
到期前用同一 venue 对、同方向开下一期的 Boros 腿，让现有 perp 腿无缝接管——免掉 perp 的平仓费与新仓开仓费。干净的续期应踩点在旧到期结算之后；提前开会产生重叠期。
_Avoid_: rollup、换仓

**Overlap（重叠期）**:
新旧两期 Boros 腿共存于同一组 perp 腿的窗口。perp 的浮动费流只有一份，两期 Boros 各自需要一份——重叠期携带一笔未对冲的浮动价差敞口（约一个名义），随旧期结算自动消失。终端会把它呈现为 boros-only/未对冲盒子，不是 bug。
_Avoid_: 双倍持仓（浮动上确实双倍，价格上依然中性）

**Liquidation domain（清算域）**:
CrossEx 统一保证金账户（perp 腿）与 Boros 抵押区（Boros 腿）是两个互不救援的独立清算域，各自有独立的风险模型与水位。任一域被清算都会把另一域的腿变成裸敞口，但两者的水位数字不可直接互比（利用率 vs 覆盖率，方向相反）。
_Avoid_: 保证金（不指明哪个域时）
